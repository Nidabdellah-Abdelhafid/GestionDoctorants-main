...
public interface MembreEquipeRepository extends JpaRepository<MembreEquipe, Long> {
    @Query(value="SELECT * FROM membre_equipe  WHERE equipe_id IN  (select id from equipe where id IN (SELECT equipe_id FROM chef_equipe WHERE   extra_user_id IN (SELECT id FROM extra_user  WHERE internal_user_id =:id) and date_fin='2000-04-28'))", nativeQuery = true)
