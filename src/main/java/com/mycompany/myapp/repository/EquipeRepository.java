...
public interface EquipeRepository extends JpaRepository<Equipe, Long> {
    ...
    @Query(value="SELECT * FROM equipe  WHERE id IN  (select equipe_id from chef_equipe where date_fin='2000-04-28' and extra_user_id IN (SELECT id FROM extra_user  WHERE internal_user_id =:id ))", nativeQuery = true)
